...
logging = setup_logger("generate_waymo_dataset", "logs/generate_waymo_dataset.log")
...
def process_lanegraph(data):
    ...
    if "roadEdge" in data["road_info"]:
        for i in range(len(data["road_info"]["roadEdge"]["id"])):
            road_edges[int(data["road_info"]["roadEdge"]["id"][i])] = data["road_info"][
                "roadEdge"
            ]["polyline"][i][:, :2]
    if "crosswalk" in data["road_info"]:
        for i in range(len(data["road_info"]["crosswalk"]["id"])):
            crosswalks[int(data["road_info"]["crosswalk"]["id"][i])] = data[
                "road_info"
            ]["crosswalk"]["polygon"][i][:, :2]
    if "stopSign" in data["road_info"]:
        for i in range(len(data["road_info"]["stopSign"]["id"])):
            stop_signs[int(data["road_info"]["stopSign"]["id"][i])] = data["road_info"][
                "stopSign"
            ]["position"][i][:2]
...
def collect_data(cfg, output_path, files_path, files, chunk):
    for c in tqdm(chunk):
        ...
        for data in dataset:
            ...
            # debug_break("Check if traffic light data is present")
            proto_list.append(proto)
            scenario_dict = MessageToDict(proto)
            scenario_list += [scenario_dict]

        for i in range(len(scenario_list)):
            proto = proto_list[i]  # ⬅️ 关键点
            ...
            for t, dmap in enumerate(proto.dynamic_map_states):
                frame_states = []
...
def main(cfg):
    ...
    n_workers = min(
        cfg.generate_waymo_dataset.get("num_workers", mp.cpu_count()), total_chunks
    )
    cfg_dict = OmegaConf.to_container(cfg, resolve=True)

    with mp.Pool(processes=n_workers) as pool:
        pool.starmap(_work_one_chunk, [(i, cfg_dict) for i in range(total_chunks)])
