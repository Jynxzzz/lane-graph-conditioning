...
def draw_lane_structure(ax, scene, ego_pos, ego_heading, radius=70,
    ...
    # Plot convention: horizontal=lateral (Y), vertical=forward (X) → swap axes
    # All plots use (bev[:, 1], bev[:, 0]) so forward points UP on screen
...
def draw_neighbors(ax, scene, ego_pos, ego_heading, anchor_frame=10, max_dist=50):
    ...
    for i, obj in enumerate(scene["objects"]):
        ...
        ax.plot(bev_pt[1], bev_pt[0], "s", color=COLOR_NEIGHBOR,
...
def draw_traffic_lights(ax, scene, ego_pos, ego_heading, frame_idx=10):
    ...
    for tl in tls[frame_idx]:
        ...
        ax.plot(pt[1], pt[0], marker="s", markersize=6, color=color,
...
def compute_dynamic_limits(gt_future, predictions, history, base_radius=70, margin=0.15):
    ...
    """
    # Collect all points (swap to plot coords: col1=lateral, col0=forward)
    all_x = [history[:, 1], gt_future[:, 1]]  # lateral
    all_y = [history[:, 0], gt_future[:, 0]]  # forward
    for k in range(predictions.shape[0]):
        all_x.append(predictions[k, :, 1])
        all_y.append(predictions[k, :, 0])
...
def draw_single_panel(ax, scene, ego_pos, ego_heading, ego_traj_bev,
    ...
    # Plot convention: horizontal=lateral (Y-BEV), vertical=forward (X-BEV)
    ...
    # 5. History trajectory (swap axes: plot lateral on x, forward on y)
    ax.plot(history[:, 1], history[:, 0], "-", color=COLOR_HISTORY,
            linewidth=2.5, alpha=0.9, zorder=8)
    ax.plot(history[-1, 1], history[-1, 0], "o", color=COLOR_HISTORY,
    ...
    for rank, k in enumerate(sorted_modes):
        ...
        ax.plot(pred_k[:, 1], pred_k[:, 0], "-", color=color,
                linewidth=lw, alpha=alpha, zorder=zorder)
        # Endpoint marker
        ax.plot(pred_k[-1, 1], pred_k[-1, 0], "o", color=color,
                markersize=6 if is_best else 3.5, alpha=alpha, zorder=zorder)

    # 7. Ground truth future — BOLD, with red star endpoint
    ax.plot(gt_future[:, 1], gt_future[:, 0], "--", color=COLOR_GT,
    ...
    ax.plot(gt_future[-1, 1], gt_future[-1, 0], "*", color="#D32F2F",
